...
_objs 	= nearestobjects [getpos player, ["Suitcase", "EvMoney"], 5];
	...
	if(_x distance player < 3 and [_x, playerarray, 3] call ISSE_ArrayNumberNear < 2)then
		...
		//Cops can not pick up illegal items (they have to take evidence)
		if (iscop and _item call INV_getitemIsIllegal) then
			{
			_action = player addaction [format ["Take evidence %1 (%2)", _name, _amount],"noscript.sqf",'player groupchat "Evidence removed."; {deletevehicle _x} foreach (nearestobjects [getpos player, ["suitcase"], 3])'];
